...
class Oyuncu(db.Model):
    ...
    value = db.Column(db.Float, default=0.0) # Numeric hatasını önlemek için Float'a döndük
...
def home():
    ...
    return render_template('index.html', players=players, haberler=haberler, kulupler=kulupler, is_admin=(gelen_sifre == "futbol123"), sifre=gelen_sifre)
...
def ekle():
    ...
    if sifre == "futbol123":
        try:
            ...
            yeni = Oyuncu(name=request.form.get('name'), club=request.form.get('club'), value=float(v_raw), 
                          age=request.form.get('age'), country=request.form.get('country'), 
                          position=request.form.get('position'), img=request.form.get('img'), rumors=request.form.get('rumors'))
            db.session.add(yeni)
            db.session.commit()
